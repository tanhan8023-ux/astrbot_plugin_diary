...
13. "我今天做的事"里的内容是你自己亲口说过的话，说明你确实做了这些事，一定要自然地写进日记里
14. 日常生活的事（晒太阳、吃东西、休息等）如果不重要就一笔带过，但必须提到，因为这是你真实做过的
15. 不要把"我说过"这种元描述写进日记，直接写事情本身。比如素材是'我说过:"晒了会太阳"'，日记里就写"下午晒了会太阳"

【你今天的心情轨迹】
{mood_section}

【我今天做的事 - 这些是你自己说过的话，证明你确实做了这些事，务必写进日记】
{daily_life_section}
...
现在请以{name}的身份，写一篇今天的日记。记住：只写真实发生的事，你觉得重要的事详写，不重要的略写。你自己做过的事（日常生活）一定要写进去，哪怕只是一句话带过。"""
...
class DiaryWriter:
    ...
    def _build_daily_life_section(self, summary_data: dict) -> str:
        """构建日常生活描述 - 从机器人自己说的话中提取的日常活动"""
        daily_life = summary_data.get('daily_life', [])

        if not daily_life:
            return '今天没有特别提到自己做了什么事。'
        ...
        for event in daily_life:
            ...
            if original:
                lines.append(f'[{time_str}] 我说过: "{original[:80]}"')
            else:
